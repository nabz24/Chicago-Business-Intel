...
func init() {
	...
	db_connection := "user=postgres dbname=chicago_business_intelligence password=root host=/cloudsql/hip-ether-442617-p9:us-central1:mypostgres sslmode=disable port = 5432"
...
func GetTaxiTrips(db *sql.DB) {
	...
	geocoder.ApiKey = "AIzaSyDioxXCBmXqPXOwvty_OdZ0NXTpOuyxZLI"
...
func GetBuildingPermits(db *sql.DB) {
	...
	// While doing unit-testing keep the limit value to 500
	// later you could change it to 1000, 2000, 10,000, etc.
	var url = "https://data.cityofchicago.org/resource/ydr8-5enu.json?$limit=2000"
...
func GetNeighborhoodData(db *sql.DB) {
	...
	for i := 0; i < len(neighborhood_data_list); i++ {
		...
		the_geom := string(neighborhood_data_list[i].The_geom)
